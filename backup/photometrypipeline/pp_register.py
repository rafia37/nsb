...
def register(filenames, telescope, sex_snr, source_minarea, aprad,
    ...
    for cat_idx, refcat in enumerate(obsparam['astrometry_catalogs']):
        ...
        open('registration_succeeded.lst', 'a').writelines("%s\n" %
                                                           '\n'.join(goodfits))
        if len(goodfits) == 0 and len(badfits) == 0:
            badfits = filenames
        open('registration_failed.lst', 'a').writelines("%s\n" %
                                                        '\n'.join(badfits))
...
###############################################################################
    ...
    image = Image.open('fgroups_1.ps')
    os.rename('fgroups_1.ps', '{}_fgroups.ps'.format(filenames[0].split('.')[0]))

    os.rename('distort_1.ps', '{}_distort.ps'.format(filenames[0].split('.')[0]))"""
