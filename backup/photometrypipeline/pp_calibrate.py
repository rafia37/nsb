...
from astropy.io.fits import getdata, getval, getheader
...
def derive_zeropoints(ref_cat, catalogs, filtername, minstars_external,
    ...
    for cat in catalogs:
        ...
        #######################################################################
        # this might be where we add color correction TODO
        #######################################################################
        clipping_steps = []
...
###############################################################################
        with open(DAN_FILENAME, 'a') as f:
            f.write('#pp zeropoint\t{}\n#pp_error\t{}'.format(
            clipping_steps[idx][0],
            clipping_steps[idx][1]
            ))
###############################################################################
...
###############################################################################
    ...
    data_header = getheader(filename)


    np_median = numpy.median(data)
    astropy_biweight = biweight_location(data)
    ast_mean, ast_median, std = sigma_clipped_stats(data, sigma=2.0, iters=20)
    mask = make_source_mask(data, snr=3, npixels=3, dilate_size=8)
    amount = numpy.count_nonzero(mask==0)
    nomask_mean, nomask_med, nomask_std = sigma_clipped_stats(data, sigma=3.0, iters=10)
    mean, median, std = sigma_clipped_stats(data, sigma=3.0, mask=mask, iters=10)

    plt.imshow(mask, origin='lower', cmap='gray')

    DAN_FILENAME = 'nsb_{}.txt'.format(filename.split('.')[0])

    plot_name = DAN_FILENAME.split('.')[0]
    plt.savefig('{}.png'.format(plot_name))
    if os.path.exists(DAN_FILENAME):
        os.remove(DAN_FILENAME)
    if not os.path.exists(DAN_FILENAME):
        with open(DAN_FILENAME, 'w') as f:
            f.write('#filename\t{}\n'.format(filename))
            f.write('#platescale_x\t{}\n'.format(data_header['SECPIXX']))
            f.write('#platescale_y\t{}\n'.format(data_header['SECPIXY']))
            f.write('#middle_jd\t{}\n'.format(data_header['MIDTIMJD']))
            f.write('#exp_time\t{}\n'.format(data_header['EXPTIME']))
            f.write('#numpy_med\t{}\n'.format(np_median))
            f.write('#ast_biweight\t{}\n'.format(astropy_biweight))
            f.write('#sc_mean\t{}\n'.format(ast_mean))
            f.write('#sc_median\t{}\n'.format(ast_median))
            f.write('#mask_mean\t{}\n'.format(mean))
            f.write('#mask_med\t{}\n'.format(median))
            ...
            f.write('#center_ra\t{}\n'.format(ra_deg))
            f.write('#center_dec\t{}\n'.format(dec_deg))
            f.write('#filter\t{}\n'.format(data_header['FILTER']))
            f.write('#ifov\t{}\n'.format(data_header['NAXIS1']))
            headers = 'id\tzp\t\t\tvar\t\t\tpp_sig\n'
            f.write(headers)
    else: pass
###############################################################################
