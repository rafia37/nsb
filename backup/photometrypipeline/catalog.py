...
class catalog(object):
    ...
    def transform_filters(self, targetfilter):
        ...
        # NSB to W
        # transformations based on Tonry et al 2012
        # TODO add link
        ...
        # NSB measurement transformation to W and z
        elif ('APASS' in self.catalogname and (targetfilter == 'W' or targetfilter == 'z')):

            logging.info('trying to transform %d %s sources to %s' %
                         (self.shape[0], self.catalogname, targetfilter))

            # z transformation based on Jester et al 2005
            # W transformation based on Tonry et al 2012
            mags = numpy.array([self['gmag'], 
                                self['rmag'],
                                self['imag'],
                                self['e_gmag'],
                                self['e_rmag'],
            ...
            # keep all since no criteria to meet
            if targetfilter == 'W':
                keep_idc = numpy.full(len(mags[0]), True)

                nmags[0] = mags[1] + 0.012 + 0.039 * (mags[0] - mags[1])
                nmags[1] = numpy.sqrt(mags[3]**2 + mags[4]**2)

            # filter true if ans < 1.15
            elif targetfilter == 'z':
                keep_idc = (1.00 * (mags[1] - mags[2]) + 0.21 ) < 1.15

                nmags[0] = mags[1] + 0.03 - 1.89 * (mags[1] - mags[2])
                nmags[1] = numpy.sqrt(mags[4]**2 + mags[5]**2)
